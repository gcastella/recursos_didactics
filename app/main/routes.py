...
from app import db
from app.main.forms import EditProfileForm, EmptyForm, ResourceForm, SearchForm
...
def user(username):
    ...
    resources = user.resources.order_by(Resource.timestamp.desc()).paginate(
        page, current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.user', username=user.username,
                       page=resources.next_num) if resources.has_next else None
    prev_url = url_for('main.user', username=user.username,
                       page=resources.prev_num) if resources.has_prev else None
    form = EmptyForm()
    return render_template('user.html', user=user, resources=resources.items,
...
def search():
    ...
    page = request.args.get('page', 1, type=int)
    resources, total = Resource.search(g.search_form.q.data, page,
                               current_app.config['POSTS_PER_PAGE'])
    next_url = url_for('main.search', q=g.search_form.q.data, page=page + 1) \
        if total > page * current_app.config['POSTS_PER_PAGE'] else None
    prev_url = url_for('main.search', q=g.search_form.q.data, page=page - 1) \
        if page > 1 else None
    return render_template('search.html', title=_('Search'), resources=resources,
